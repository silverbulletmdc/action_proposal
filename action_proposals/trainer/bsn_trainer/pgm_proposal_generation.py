...
class TemHandler(VideoRecordHandler):
    def __init__(self, tem_csv_dir: str, subset: str = "training"):
        r"""
        TEM feature handler.
    ...
    def __call__(self, video_record: VideoRecord):
        path = os.path.join(self._tem_csv_dir, '{}.csv'.format(video_record.video_name))
        df = pd.read_csv(path)

        return df.values[:, [1,0,2]], video_record


def get_tem_dataset(tem_csv_dir: str, json_path: str, video_info_new_csv_path: str, subset: str):
    video_record_handler = TemHandler(tem_csv_dir, subset)
...
def sub_proc(q: mp.Queue, cfg: argparse.Namespace, datasets):
    while True:
        subset, idx = q.get(block=True)
        scores, video_record = datasets[subset][idx]
...
def main():
    ...
    anet = cfg.anet
    train_dataset = get_tem_dataset(cfg.tem.tem_csv_dir, anet.json_path, anet.video_info_new_csv_path, "training")
    val_dataset = get_tem_dataset(cfg.tem.tem_csv_dir, anet.json_path, anet.video_info_new_csv_path, "validation")
    ...
    for i in range(cfg.pgm.proposal_workers):
        proc = mp.Process(target=sub_proc, args=(queue, cfg, {"training": train_dataset, "validation": val_dataset}))
        proc.start()
        procs.append(proc)
