...
class PgmFeatureHandler(VideoRecordHandler):
    def __init__(self, tem_results_file: str, proposal_csv_path: str, subset: str = "training"):
        r"""
        PGM feature handler.
        ...
        self.proposal_csv_path = proposal_csv_path

    def __call__(self, video_record: VideoRecord) -> Tuple[np.ndarray, VideoRecord, np.ndarray]:
        """

        :param video_record:
        :return: scores [3, L] (start, actionness, end),
                 video record,
                 proposals [N, 6] (xmin, xmax, min_score, max_score, iou, ioa)
        """
        proposals_path = os.path.join(self.proposal_csv_path, '{}.csv'.format(video_record.video_name))
        proposals = pd.read_csv(proposals_path)
        return self.tem_scores[video_record.idx], video_record, proposals.values


def get_pgm_feature_dataset(tem_results_file: str, proposal_csv_path: str, json_path: str, video_info_new_csv_path: str,
                            subset: str):
    video_record_handler = PgmFeatureHandler(tem_results_file, proposal_csv_path, subset)
...
def generate_feature(scores: np.ndarray, video_record: VideoRecord, proposals: np.ndarray, subset: str) -> np.ndarray:
    ...
    :param scores: [3, L] (start, actionness, end),
    ...
    _, score_action, _ = scores
    tscale = scores.shape[1]
    tgap = 1.0 / tscale
